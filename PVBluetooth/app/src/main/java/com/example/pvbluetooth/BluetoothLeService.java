...
public class BluetoothLeService extends Service {
    ...
    BluetoothLeService(Context context, String macAddress){
        m_context = context;
    ...
    private final BluetoothGattCallback m_gattCallback =
            new BluetoothGattCallback() {
                ...
                public void onConnectionStateChange(BluetoothGatt gatt, int status,
                    ...
                    Intent intent = new Intent(SaticResources.BROADCAST_NAME_CONNECTION_UPDATE);
                    if (newState == BluetoothProfile.STATE_CONNECTED) {
                        intent.putExtra(SaticResources.EXTRAS_CONNECTION_STATE, SaticResources.CONNECTION_STATE_CONNECTED);
                        DiscoverServicesAfterDelay(gatt);

                    }
                    else if (newState == BluetoothProfile.STATE_DISCONNECTED) {
                        intent.putExtra(SaticResources.EXTRAS_CONNECTION_STATE, SaticResources.CONNECTION_STATE_DISCONNECTED);
                    }
                    else if (newState == BluetoothProfile.STATE_CONNECTING) {
                        intent.putExtra(SaticResources.EXTRAS_CONNECTION_STATE, SaticResources.CONNECTION_STATE_CONNECTING);
                    }
                ...
                public void onServicesDiscovered(BluetoothGatt gatt, int status) {
                    Intent intent = new Intent(SaticResources.BROADCAST_NAME_SERVICES_DISCOVERED);
                    ...
                    if (status == BluetoothGatt.GATT_SUCCESS)
                    {
                        ...
                        intent.putExtra(SaticResources.EXTRAS_SERVICES_DISCOVERED,
                                foundSuccess);
                    }
                    else // Many reasons it could fail to find services
                    {
                        intent.putExtra(SaticResources.EXTRAS_SERVICES_DISCOVERED,
                                SaticResources.SERVICES_DISCOVERY_GENERAL_FAILURE);
                    }
                    m_context.sendBroadcast(intent);
                }
                ...
                public void onCharacteristicChanged(BluetoothGatt gatt,
                    ...
                    Intent intent = new Intent(SaticResources.BROADCAST_NAME_TX_CHARATERISTIC_CHANGED);
                    intent.putExtra(SaticResources.EXTRAS_TX_DATA, txData);
                    m_context.sendBroadcast(intent);
